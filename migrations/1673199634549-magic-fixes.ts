...
export class magicFixes1673199634549 implements MigrationInterface {
    name = 'magicFixes1673199634549'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`test\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NULL, \`testCode\` varchar(255) NOT NULL, \`duration\` varchar(255) NULL, \`rate\` int NULL, \`testType\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`visit\` (\`id\` varchar(36) NOT NULL, \`unique\` varchar(255) NOT NULL, \`visitNote\` varchar(255) NULL, \`allergy\` json NULL, \`vitalSigns\` json NULL, \`assessmentLog\` json NULL, \`recommendation\` json NULL, \`doctorNote\` json NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`patientId\` varchar(36) NULL, UNIQUE INDEX \`IDX_79d896858972e3fce4fd433add\` (\`unique\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`patient\` (\`id\` varchar(36) NOT NULL, \`firstName\` varchar(255) NOT NULL, \`lastName\` varchar(255) NOT NULL, \`middleName\` varchar(255) NULL, \`email\` varchar(255) NOT NULL, \`phoneNumber\` varchar(255) NULL, \`residentialAddress\` text NULL, \`permanentAddress\` text NULL, \`nextOfKin\` text NULL, \`payerDetails\` text NULL, \`religion\` varchar(255) NULL, \`occupation\` varchar(255) NULL, \`admissionStatus\` enum ('PENDING', 'ADMITTED', 'DISCHARGED', 'OUTPATIENT', 'DEAD') NOT NULL DEFAULT 'OUTPATIENT', \`language\` varchar(255) NULL, \`bloodGroup\` enum ('A_POSITIVE', 'A_NEGATIVE', 'B_POSITIVE', 'B_NEGATIVE', 'AB_POSITIVE', 'AB_NEGATIVE', 'O_POSITIVE', 'O_NEGATIVE') NULL, \`genotype\` enum ('AA', 'AS', 'SS', 'AC', 'CC', 'SC') NULL, \`unique\` varchar(255) NOT NULL, \`reasonForDeath\` varchar(255) NULL, \`dateOfBirth\` varchar(255) NULL, \`nationality\` varchar(255) NULL, \`profilePicture\` varchar(255) NULL, \`maritalStatus\` enum ('SINGLE', 'MARRIED', 'DIVORCED', 'WIDOWED', 'SEPARATED', 'ENGAGED') NOT NULL, \`dischargeDates\` text NULL, \`admissionDates\` text NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`user\` ADD \`genotype\` enum ('AA', 'AS', 'SS', 'AC', 'CC', 'SC') NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`user\` DROP COLUMN \`genotype\``);

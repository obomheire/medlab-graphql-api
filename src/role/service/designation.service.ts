...
export class DesignationService {
    ...
    create = async (createDesignationInput: CreateDesignationInput): Promise<Designation> => {
        try {
            const designation = this.designationRepository.create(createDesignationInput);
    ...
    findOne = async (id: string): Promise<Designation> => {
        try {
            return await this.designationRepository.findOne({ where: { id }, relations: [ 'role' ] });
        } catch (error) {
            throw error;
        }
    }

    update = async (id: string, updateDesignationInput: UpdateDesignationInput): Promise<Designation> => {
        try {

...
export class ChatHistoryService {
  ...
  countTokens(
    ...
    const text = chatHistory.reduce((acc, entry) => acc + entry.content, '');

    return isWithinTokenLimit(text, tokenLimit);
  }

  // Truncate older messages to stay within a token limit
  reduceChatHistory(userUUID: string) {
    const chatHistory = this.getChatHistory(userUUID);
